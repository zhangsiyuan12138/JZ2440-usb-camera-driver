...
static int uvc_register_video(struct uvc_device *dev)
{
	...
	vdev = video_device_alloc();                                       /*申请video_device结构体*/
	if (vdev == NULL)
		return -1;
	...
	vdev->minor = -1;                                    /* 设置video_device的次设备号  */
	vdev->fops = &uvc_fops;                              /* 设置video_device的file_operations  */
	 ...
	 */
	dev->video.vdev = vdev;                       /*  连接video_device和uvc_device.       (uvc_device->uvc_video_device->video_device = video_device)   */
	video_set_drvdata(vdev, &dev->video);

	if (video_register_device(vdev, VFL_TYPE_GRABBER, -1) < 0) {        /* 注册video_device结构体  */
		dev->video.vdev = NULL;
		     ...
		     const struct usb_device_id *id)
{
	...
	/* Register the video devices. */
	if (uvc_register_video(dev) < 0)               /* 注册uvc_device设备（实为注册video_device设备）   */
		goto error;
